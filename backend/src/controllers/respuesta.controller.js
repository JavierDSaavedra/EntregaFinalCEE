...
export async function getResultadosPregunta(req, res) {
  try {
    ...
    res.status(200).json({ success: true, data: conteo });
  } catch (error) {
...
export async function getRespuestas(req, res) {
  try {
    const respuestaRepository = AppDataSource.getRepository(Respuesta);
    const respuestas = await respuestaRepository.find({ relations: ["usuario", "pregunta", "votacion"] });
    res.status(200).json({ success: true, message: "Respuestas obtenidas correctamente", data: respuestas });
  } catch (error) {
...
export async function createRespuesta(req, res) {
  try {
    ...
    if (error) {
      return res.status(400).json({ success: false, message: "Error de validación", details: error.details.map(d => ({ field: d.path[0], message: d.message })) });
    }
    if (!req.user?.id) {
    ...
    if (value.usuarioId && value.usuarioId !== usuarioId && !esAdmin) {
      return res.status(403).json({ success: false, message: "No tienes permiso para responder por otro usuario" });
    }
    const preguntaRepository = AppDataSource.getRepository(Pregunta);
    ...
    if (!pregunta || !votacion) {
      return res.status(404).json({ success: false, message: !pregunta ? "Pregunta no encontrada" : "Votación no encontrada" });
    }
    if (pregunta.votacionId !== votacion.votacionId) {
      return res.status(400).json({ success: false, message: "La pregunta no pertenece a la votación especificada" });
    }
    const respuestaRepository = AppDataSource.getRepository(Respuesta);
    ...
    res.status(201).json({ success: true, message: "Respuesta registrada exitosamente", data: nuevaRespuesta });
  } catch (error) {
...
export async function updateRespuesta(req, res) {
  try {
    ...
    const respuestaRepository = AppDataSource.getRepository(Respuesta);
    const existingRespuesta = await respuestaRepository.findOneBy({ respuestaId: parseInt(id) });
    ...
    if (!esAdmin && !esDueño) {
      return res.status(403).json({ success: false, message: "No tienes permiso para modificar esta respuesta" });
    }
    ...
    res.status(200).json({ success: true, message: "Respuesta actualizada correctamente", data: updatedRespuesta });
  } catch (error) {
    res.status(500).json({ success: false, message: "Error interno al actualizar respuesta", error: error.message });
  }
}
...
export async function deleteRespuesta(req, res) {
  try {
    ...
    const respuestaRepository = AppDataSource.getRepository(Respuesta);
    const respuesta = await respuestaRepository.findOneBy({ respuestaId: parseInt(id) });
    if (!respuesta) {
      return res.status(404).json({ success: false, message: "Respuesta no encontrada" });
    }
    const esAdmin = req.user.role === 'administrador';
    ...
    res.status(200).json({ success: true, message: "Respuesta eliminada correctamente" });
  } catch (error) {
    res.status(500).json({ success: false, message: "Error interno al eliminar respuesta", error: error.message });
  }
}
