...
export async function register(req, res) {
  try {
    ...
    const { username, rut, email, password, Generacion, role } = req.body;
    ...
    res.status(201).json({ message: "Usuario registrado exitosamente!", data: dataUser });
  } catch (error) {
...
export async function login(req, res) {
  try {
    ...
    const payload = {
      ...
      rut: userFound.rut,
      role: userFound.role,
      Generacion: userFound.Generacion
    };
