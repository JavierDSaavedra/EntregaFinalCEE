...
import { preguntaValidations, preguntaUpdateValidations } from "../validations/pregunta.validation.js";
...
export async function getPreguntas(req, res) {
  try {
    ...
    if (req.query.votacionId) {
      ...
      preguntas = await preguntaRepository.find({ relations: ["votacion", "respuestas"] });
    }
    res.status(200).json({ success: true, message: "Preguntas obtenidas correctamente", data: preguntas });
  } catch (error) {
...
export async function createPregunta(req, res) {
  try {
    ...
    const votacion = await votacionRepository.findOneBy({ votacionId: req.body.votacionId });
    if (!votacion) {
      return res.status(404).json({ success: false, message: "La votación especificada no existe" });
    }
    ...
    res.status(201).json({ success: true, message: "Pregunta creada correctamente", data: newPregunta });
  } catch (error) {
    res.status(500).json({ success: false, message: "Error interno al crear pregunta", error: error.message });
  }
  }
...
export async function updatePregunta(req, res) {
  try {
    ...
    const preguntaRepository = AppDataSource.getRepository(Pregunta);
    const pregunta = await preguntaRepository.findOneBy({ preguntaId: parseInt(id) });
    if (!pregunta) {
    ...
    res.status(200).json({ success: true, message: "Pregunta actualizada correctamente", data: updatedPregunta });
  } catch (error) {
    res.status(500).json({ success: false, message: "Error interno al actualizar pregunta", error: error.message });
  }
  }
...
export async function deletePregunta(req, res) {
  try {
    const { id } = req.params;
    const preguntaRepository = AppDataSource.getRepository(Pregunta);
    const pregunta = await preguntaRepository.findOne({ where: { preguntaId: parseInt(id) }, relations: ["respuestas"] });
    if (!pregunta) {
      return res.status(404).json({ success: false, message: "Pregunta no encontrada" });
    }
    if (pregunta.respuestas && pregunta.respuestas.length > 0) {
      return res.status(400).json({ success: false, message: "No se puede eliminar una pregunta con respuestas asociadas" });
    }
    await preguntaRepository.remove(pregunta);
    res.status(200).json({ success: true, message: "Pregunta eliminada correctamente" });
  } catch (error) {
    res.status(500).json({ success: false, message: "Error interno al eliminar pregunta", error: error.message });
  }
}