...
export async function cambiarRolUsuario(req, res) {
  try {
    ...
    if (!rolesPermitidos.includes(nuevoRol)) {
      return res.status(400).json({ message: "Rol no válido. Roles válidos: estudiante, tesorero, secretario, presidente." });
    }

    usuario.role = nuevoRol;
    await userRepo.save(usuario);

    res.status(200).json({ message: "Rol actualizado correctamente.", data: { id: usuario.id, nuevoRol } });
  } catch (error) {
