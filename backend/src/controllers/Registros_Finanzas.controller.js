  ...
  try {
    ...
    if (!id && !Nombre_Transaccion) {
      return res.status(400).json({
        mensaje: "Debe proporcionar el ID o el Nombre de la transacción para actualizar.",
      });
    }
...
export const deleteRegistro = async (req, res) => {
  try {
    ...
    if (!id && !Nombre_Transaccion) {
      return res.status(400).json({
        mensaje: "Debe proporcionar el ID o el Nombre de la transacción para eliminar.",
      });
    }
...
export const getInformeRegistros = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      mensaje: "Informe de registros obtenido exitosamente",
      data: registros,
      totalIngresos,
      totalEgresos,
      totalNeto
